...
app = FastAPI(title="Enhanced Financial Projection API with Complete Local Storage", version="3.2.0")
...
FINANCE_DB_PASSWORD = "Utkar$h007"
...
PROJECTIONS_DB_PASSWORD = "Utkar$h007"
PROJECTIONS_DB_PORT = 5432

# Google Gemini API Configuration - Multiple Keys for Load Balancing
GEMINI_API_KEYS = [
    ...
    "AIzaSyAs5pxkiE9uj6YJIm4IsGbE_3-vQEpIoaE",  # Back_Fin16
    "AIzaSyBzPkosDPwRAqPLtWQ55-CG9r-muoFFIY0",  # Back_Fin17
    "AIzaSyCl4FUhojs0w9kHoepPWG49BzGLFzQ1ESA",  # Back_Fin18
    "AIzaSyCPkFeYPB_r1UCoyiVDsfeorlFLLrN6Wow",  # Back_Fin19
    "AIzaSyD0Xk87FtNC8IFTOpU9yKbmrHy7bQa0KNI"  # Back_Fin20
...
logger.info("Initializing Gemini client on startup...")
...
def get_azure_container_name(user_id: str) -> str:
    """Get Azure container name for Finance user with complete logging"""
    logger.info(f"🔍 CONTAINER LOOKUP: Starting lookup for user_id: {user_id}")
    
    # Store the lookup request
    container_lookup_request = {
        "user_id": user_id,
        "operation": "get_azure_container_name",
        "timestamp": datetime.now().isoformat()
    }
    save_to_local_storage(container_lookup_request, 'database_queries', 'container_lookup_request', user_id)
    
    conn = None
    try:
        logger.debug("📡 Establishing PostgreSQL connection for container lookup...")
        ...
        with conn.cursor() as cur:
            sql = """
                ...
                LIMIT 1
            """
            logger.debug(f"🔎 Executing SQL query for client_id: {user_id}")
            ...
            query_data = {
                ...
                "parameters": [user_id],
                "user_id": user_id,
                "database": ONBOARDING_DB_NAME
            }
            save_to_local_storage(query_data, 'database_queries', 'container_lookup_sql', user_id)
            ...
            query_result = {
                "user_id": user_id,
                "query_executed": sql,
                "result_found": row is not None,
                "container_name": row[0] if row else None
            }
            save_to_local_storage(query_result, 'database_queries', 'container_lookup_result', user_id)
            
            if not row:
                logger.warning(f"⚠️ No container found for user_id={user_id}")
                error_data = {
                    "error_type": "container_not_found",
                    "user_id": user_id,
                    "message": f"No Azure container found for client_id: {user_id}"
                }
                save_to_local_storage(error_data, 'errors', 'container_not_found', user_id)
                raise HTTPException(status_code=404, detail=f"No Azure container found for client_id: {user_id}")
            
            container_name = row[0]
            logger.info(f"✓ Container found for user_id={user_id}: {container_name}")
            
            # Store successful lookup
            success_data = {
                "user_id": user_id,
                "container_name": container_name,
                "operation": "successful_container_lookup"
            }
            save_to_local_storage(success_data, 'database_queries', 'container_lookup_success', user_id)
            
            return container_name
    
    except psycopg2.Error as e:
        logger.error(f"💥 Database error retrieving container: {str(e)}")
        db_error_data = {
        ...
        save_to_local_storage(db_error_data, 'errors', 'database_error', user_id)
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    except Exception as e:
        logger.error(f"💥 Error retrieving container from DB: {str(e)}")
        general_error_data = {
        ...
        save_to_local_storage(general_error_data, 'errors', 'general_error', user_id)
        raise HTTPException(status_code=500, detail=f"Error retrieving container: {str(e)}")
...
async def root():
    ...
    root_access_data = {
        ...
        "message": "Enhanced Financial Projection API v3.2 with Complete Local Storage"
    }
    save_to_local_storage(root_access_data, 'system_logs', 'root_access')
    
    return {"message": "Enhanced Financial Projection API v3.2 with Complete Local Storage is running"}
...
"""
        ...
        # Step 4: Make API calls with retry logic
        max_retries = 3
        retry_count = 0
        response = None
        last_error = None
        
        logger.info(f"🚀 Starting API calls with {max_retries} retry attempts")
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                logger.info(f"🔄 Attempt {retry_count + 1}/{max_retries} - Using deterministic API key index: {key_index}")
                ...
                logger.info("📡 Making API call to Gemini 2.5 Pro...")
...
async def test_azure_connection(client_id: str):
    ...
    try:
        ...
        logger.debug("Listing blobs in financial intelligence report folder")
        # List blobs in the financial intelligence report folder
        folder_prefix = "financial intelligence report/"
...
logger.info("Enhanced Financial Projection API v3.2 with Complete Local Storage initialized")
logger.info(f"Available Gemini API keys: {len(GEMINI_API_KEYS)}")
